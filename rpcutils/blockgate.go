...
type BlockGetter interface {
	...
	BestBlock() (*hxutil.Block, error)
	Block(chainhash.Hash) (*hxutil.Block, error)
...
// directly from hxd via RPC and subsequently update the internal block cache
// with the retrieved block.
type MasterBlockGetter interface {
	BlockGetter
	UpdateToBestBlock() (*hxutil.Block, error)
	UpdateToNextBlock() (*hxutil.Block, error)
	UpdateToBlock(height int64) (*hxutil.Block, error)
}
...
type BlockGate struct {
	...
	blockWithHash map[chainhash.Hash]*hxutil.Block
...
func NewBlockGate(client *rpcclient.Client, capacity int) *BlockGate {
	return &BlockGate{
		...
		blockWithHash: make(map[chainhash.Hash]*hxutil.Block),
...
// NodeHeight gets the chain height from hxd.
...
func (g *BlockGate) BestBlock() (*hxutil.Block, error) {
	g.RLock()
	defer g.RUnlock()
	var err error
	hash, ok := g.hashAtHeight[g.height]
...
func (g *BlockGate) Block(hash chainhash.Hash) (*hxutil.Block, error) {
	g.RLock()
	defer g.RUnlock()
	var err error
	block, ok := g.blockWithHash[hash]
...
func (g *BlockGate) UpdateToBestBlock() (*hxutil.Block, error) {
	_, height, err := g.client.GetBestBlock()
	if err != nil {
...
// updates the cache.
func (g *BlockGate) UpdateToNextBlock() (*hxutil.Block, error) {
	g.Lock()
	height := g.height + 1
	g.Unlock()
	return g.UpdateToBlock(height)
...
// hxd and stores it in cache.
func (g *BlockGate) UpdateToBlock(height int64) (*hxutil.Block, error) {
	g.Lock()
	defer g.Unlock()
	return g.updateToBlock(height)
}
