...
func (c *insightApiContext) TxConverterWithParams(txs []*hxjson.TxRawResult, noAsm bool, noScriptSig bool, noSpent bool) ([]apitypes.InsightTx, error) {
	...
	for _, tx := range txs {
			...
			if err == nil {
				if len(addresses) > 0 {
					...
					if tx.Confirmations == 0 {
						txNew.Vins[vinID].Value = hxutil.Amount(value).ToCoin()
					}
					// Update Vin due to HXD AMOUNTIN - END
					txNew.Vins[vinID].Addr = addresses[0]
				}
			}
			dcramt, _ := hxutil.NewAmount(txNew.Vins[vinID].Value)
		...
		dcramt, _ := hxutil.NewAmount(vOutSum)
		txNew.ValueOut = dcramt.ToCoin()

		dcramt, _ = hxutil.NewAmount(vInSum)
		txNew.ValueIn = dcramt.ToCoin()

		dcramt, _ = hxutil.NewAmount(txNew.ValueIn - txNew.ValueOut)
